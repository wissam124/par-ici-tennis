...
export const formatAvailabilityHour = hour => String(hour).padStart(2, '0')

export const getAvailabilitySlotSelector = (date, hour) => {
  const paddedHour = formatAvailabilityHour(hour)

  return `[datedeb="${date.format('YYYY/MM/DD')} ${paddedHour}:00:00"]`
}
...
const searchLocationOnce = async (page, location, date, hours, onProgress) => {
  ...
  for (const hour of hours) {
    ...
    const slotSelector = getAvailabilitySlotSelector(date, normalizedHour)
