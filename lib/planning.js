...
const BOOKING_TIMESTAMP_PATTERN = /\b\d{2}\.\d{2}\.\d{4}\s+\d{2}:\d{2}\b/
...
export const createPlanningSlots = ({ courts, rows }, location, date) => {
  ...
  for (const row of rows) {
    for (const [index, cell] of row.cells.entries()) {
      ...
      const normalizedDetails = details.length > 0 ? normalizeText(details.join(' ')) : ''
      const bookingTimestamp = normalizedDetails.match(BOOKING_TIMESTAMP_PATTERN)?.[0]
      slots.push({
        location,
